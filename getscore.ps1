...
param ($reportfile)
Write-Host "Script adapted from Elliot Munro published on https://gcits.com/knowledge-base/export-customers-microsoft-secure-scores-to-csv-and-html-reports/" -ForegroundColor Green

$externalCSS = "<link rel=`"stylesheet`" href=`"https://dl.dropbox.com/s/vpx9ysgr11cah4u/reports.css?dl=0`">"
$yourLogo = "https://www.uneedsecurity.com/wp-content/uploads/2020/11/Logo_w_250x90.svg"
...
# Set the $appIDUri variable to a use a valid domain in your tenant. eg. https://www.uneedsecurity.com/$((New-Guid).ToString())
 
$homePage = "https://www.uneedsecurity.com"
$appIdURI = "https://www.uneedsecurity.com/$((New-Guid).ToString())"
        ...
        do {
            try {
                ...
                if ($scores) {
                    ...
                    $reportByLine = "<div>Secure Score report compiled by <strong>$tenantName</strong> on $((Get-Date).ToLongDateString() + " " + (Get-Date).ToShortTimeString())</div>"
                    ...
                    # "$externalCSS <img class=`"float-right`" src=$yourLogo> $($completeReport -join "<p></p>")<img class=`"float-right`" src=$yourLogo><br><br><div>Report compiled by <strong>$tenantName</strong> on $((Get-Date).ToLongDateString()) + " " + $((Get-Date).ToShortTimeString())</div>" | Out-file C:\temp\securescorereports\$($contract.DefaultDomainName).html
                    "$externalCSS <img class=`"float-right`" src=$yourLogo> $($completeReport -join "<p></p>")<img class=`"float-right`" src=$yourLogo><br><br><div>Report compiled by <strong>$tenantName</strong> on $((Get-Date).ToLongDateString() + " " + (Get-Date).ToShortTimeString())</div>" | Out-file $reportfile
